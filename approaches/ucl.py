...
class Appr(object):
    ...
    def __init__(self, model, nepochs=100, sbatch=256, lr=0.001, 
        ...
        file_name = log_name
        self.logger = utils.logger(file_name=file_name, resume=False, path='./result_data/', data_format='csv')
    ...
    def train_epoch(self,t,x,y):
        ...
        for i in range(0, len(r), self.sbatch):
            ...
            loss = self.custom_regularization(self.model_old, self.model, mini_batch_size, t, loss)
            # Backward
...
# custom regularization
    ...
    def custom_regularization(self, saver_net, trainer_net, mini_batch_size, t, loss=None):
        ...
        for (_, saver_layer), (_, trainer_layer) in zip(saver_net.named_children(), trainer_net.named_children()):
            ...
            sigma_weight_reg_sum = sigma_weight_reg_sum + (weight_sigma - torch.log(weight_sigma)).sum() # (3b) 
            sigma_weight_normal_reg_sum = sigma_weight_normal_reg_sum + (normal_weight_sigma - torch.log(normal_weight_sigma)).sum() #(6)
            ...
            if t == 0:
                loss = loss - 10*(trainer_weight_sigma.norm(2))**2
