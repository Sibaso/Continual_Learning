...
def KL_Powerspherical(q_dist, p_dist):
    # if (q_dist.loc == p_dist.loc).all() and (q_dist.scale == p_dist.scale).all():
    #     return torch.zeros(q_dist.scale.shape)
    sample = q_dist.rsample()
    kld = - q_dist.entropy() - p_dist.log_prob(sample)
    # if (kld < 0).any():
    #     print('entropy', q_dist.entropy().sum())
    #     print('log prob', p_dist.log_prob(sample).sum())
    #     raise RuntimeError('KL divergence of PS : %d negative values out of %d from %.6E' % ((kld < 0).sum(), kld.numel(), torch.min(kld)))
    # if (kld != kld).any():
    #     raise RuntimeError('KL divergence of PS : %d nan values out of %d from %.6E' % ((kld != kld).sum(), kld.numel(), torch.min(kld)))

    return kld

def KL_Powerspherical_vMF(powerspherical_dist, vMF_scale, log_vMF_normalizer):
    dim = powerspherical_dist.loc.size(-1)
    alpha = powerspherical_dist.base_dist.marginal_t.base_dist.concentration0
    beta = powerspherical_dist.base_dist.marginal_t.base_dist.concentration1
    kld = - powerspherical_dist.entropy() + log_vMF_normalizer - vMF_scale*((alpha-beta)/(alpha+beta))
    # print(powerspherical_dist.log_normalizer())
    if (kld != kld).any():
        raise RuntimeError('KL divergence of vMF : %d nan values out of %d from %.6E' % ((kld != kld).sum(), kld.numel(), torch.min(kld)))

    # if (kld < 0).any():
    #     warnings.warn('KLD has negative value %f' % float(torch.min(kld)))
...
class KL_vMF_kappa_Function_full(Function):

    @staticmethod
    def forward(ctx, q_loc, q_concentration, p_loc, p_concentration, reg_strength):
        # q_loc = q_loc.clone().detach().requires_grad_(False)
        # q_concentration = q_concentration.clone().detach().requires_grad_(False)
        # p_loc = p_loc.clone().detach().requires_grad_(False)
        # p_concentration = p_concentration.clone().detach().requires_grad_(False)
        assert q_loc.size()[:-1] == q_concentration.size()
        assert q_concentration.size() == p_concentration.size()
        dim = q_loc.size(-1)
        nu = float(dim / 2.0)
        ...
        bessel_ratio_lower, bessel_ratio_upper = _bessel_ratio_bound(nu, q_concentration)
        bessel_ratio = (bessel_ratio_lower + bessel_ratio_upper) / 2.0
        term_bessel_ratio = (q_concentration - p_concentration * (reg_strength*p_loc*q_loc).sum(-1)) * bessel_ratio
        term_log_kappa = (nu - 1.0) * (torch.log(q_concentration) - torch.log(p_concentration))
        log_bessel = _approximate_log_iv_series(nu - 1.0, torch.stack([q_concentration, p_concentration], dim=0))
        term_log_bessel = log_bessel[0] - log_bessel[1]
        kld = term_bessel_ratio + term_log_kappa - term_log_bessel
        ...
        if (kld < -1e-1).any():
            ...
            print(kld)
            raise RuntimeError('KL divergence of vMF is negative.')
        ctx.intermediate = (q_loc, q_concentration, p_loc, p_concentration, bessel_ratio_lower, bessel_ratio_upper, q_loc.new_full((1,), dim), reg_strength)
        return kld

    @staticmethod
    def backward(ctx, grad_output):
        q_loc, q_concentration, p_loc, p_concentration, bessel_ratio_lower, bessel_ratio_upper, dim, reg_strength = ctx.intermediate
        nu = float(dim / 2.0)
        grad_q_loc = grad_q_concentration = grad_p_loc = grad_p_concentration = None

        if ctx.needs_input_grad[0]:
            bessel_ratio = (bessel_ratio_lower + bessel_ratio_upper) / 2.0
            grad_q_loc = - p_concentration.unsqueeze(-1) * reg_strength * p_loc * bessel_ratio.unsqueeze(-1)
            grad_q_loc = grad_output.unsqueeze(-1) * grad_q_loc
        if ctx.needs_input_grad[1]:
            ratio1_lower_bound, ratio1_upper_bound = _bessel_ratio_bound(nu + 1.0, q_concentration)
            ratio0_lower_bound, ratio0_upper_bound = bessel_ratio_lower, bessel_ratio_upper

            kappa_const = (q_concentration - p_concentration * (reg_strength*p_loc*q_loc).sum(-1)) / 2.0
            upper_bound = kappa_const * (ratio0_upper_bound * (ratio1_upper_bound - 2 * ratio0_lower_bound - (2 * nu - 2) / q_concentration) + 1.0)
            lower_bound = kappa_const * (ratio0_lower_bound * (ratio1_lower_bound - 2 * ratio0_upper_bound - (2 * nu - 2) / q_concentration) + 1.0)
            grad_estimate = (upper_bound + lower_bound) / 2.0
        ...
        return grad_q_loc, grad_q_concentration, grad_p_loc, grad_p_concentration
...
KL_vMF_kappa_full = KL_vMF_kappa_Function_full.apply
